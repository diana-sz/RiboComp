...
"""
Run RBA with parameters that make RNA more expensive than proteins
"""
...
from general_functions import make_nice_results, get_const_parameters, make_matrix, bisection_search_mu

k1, k2, k3, k5, c, kel, n1, n2, n3, n4, n5, Mtot, mwG, mwAA, mwNT, cell_mass, avogadro = get_const_parameters()

# adjust parameters to make RNA more expensive than proteins
kel = kel*3
n4 = n4*15
c = c/10

prot_fractions = np.arange(0.01, 1, 0.01)  # protein fractions to test
mus = np.arange(2.5,5,0.001) # growth rates to test

last_mus = []
prog = 0
plt.title("RBA reverse")
...
res.to_csv("../data/01_RBA_reverse.csv")
