...
fluxes_bremer <- read.csv("data/fluxes_bremer.csv", row.names = 1)
fluxes1 <- read.csv("data/05_fluxes_RNAPmax.csv", row.names = 1)
fluxes2 <- read.csv("data/06_fluxes_RNAPmax_noacc.csv", row.names = 1)
fluxes3 <- read.csv("data/07_fluxes_RNAPmax_noacc_act.csv", row.names = 1)
...
plot_RNAP <- function(data, data2, xlim, cex_labs, cex_axis, col_fluxes){
  ...
  plot(vRNAP ~ mu, data, log = "y", #type = "l", lwd = 7,
       ylim = ylim, xlim = xlim,
...
png(filename = "plots/fluxes_RNAPmax.png", type="cairo", units="cm", 
    width=fig_size[1], height=fig_size[2], res=300)
plot_RNAP(fluxes1, fluxes_bremer, c(0,3.5), cex_labs, cex_axis, uni_blue)
dev.off()


png(filename = "plots/fluxes_RNAPmax_noacc.png", type="cairo", units="cm", 
    width=fig_size[1], height=fig_size[2], res=300)
plot_RNAP(fluxes2, fluxes_bremer, c(0,3.5), cex_labs, cex_axis, uni_blue)
dev.off()


png(filename = "plots/fluxes_RNAPmax_noacc_act.png", type="cairo", units="cm", 
    width=fig_size[1], height=fig_size[2], res=300)
