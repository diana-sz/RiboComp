...
deg_rates_dic = {
    "R_deg": np.arange(20, 22, 0.1),
    "R_deg_hill-6": np.arange(112, 113, 0.1),
    "R_deg_hill-2": np.arange(35.5, 37, 0.1)
}

for matrix_type, deg_rates in deg_rates_dic.items():
    for k_deg in deg_rates:
        ...
        if abs(xrp_opt - 0.36) <= 0.001:
            to_print = f"(matrix={matrix_type}): {k_deg:.2f}"
